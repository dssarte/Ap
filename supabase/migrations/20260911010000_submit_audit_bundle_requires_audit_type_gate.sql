-- Store-manager accounts were able to submit QA-only checklists
-- (audit_templates.requires_audit_type = true) even though the app's own
-- template list correctly hides those templates from anyone outside the
-- Quality Assurance department. The list filter is client-side only —
-- submit_audit_bundle never checked who was allowed to submit a given
-- template, just that the submitter matched their own JWT and the template
-- was active. This adds that check server-side, using the existing
-- private.is_qa_or_admin() helper, so it's enforced no matter which client
-- (web, mobile, or a raw API call) sends the submission.
